    ...
    encode: (string, offset) => {
        ...
        for (let i = 0; i < string.length; i++) { //inicio recorrido de caracteres
            ...
            if (letterE.match(/[a-z]/i)) { //identificacion de espacios vacios 
                if (string.charCodeAt(i) >= 65 && string.charCodeAt(i) <= 90) { //Devuelve un valor assci
                    ...
                    cipher += String.fromCharCode(textChar); //Devuelve una cadena creada en assci
                }
                } else {
                cipher += letterE;
            }
        ...
        for (let i = 0; i < string.length; i++) { //inicio recorrido de caracteres
            ...
            if (letterD.match(/[a-z]/i)) { //identificacion de espacios vacios 
                if (string.charCodeAt(i) >= 65 && string.charCodeAt(i) <= 90) { //Devuelve un valor assci
                    ...
                    decipher += String.fromCharCode(textChar); //Devuelve una cadena creada en assci
                }
                } else {
                decipher += letterD;
            }
